celevator.doors.centermetal = {}

local S = core.get_translator("celevator")

core.register_node("celevator:hwdoor_left_steel_bottom",{
	description = S("Steel Hoistway Door (left, bottom - you hacker you!)"),
	tiles = {
	...
	drop = "celevator:hwdoor_centersteel",
	paramtype = "light",
	paramtype2 = "4dir",
	drawtype = "nodebox",
	node_box = {
	...
	_celevator_doortype = "centermetal",
})

core.register_node("celevator:hwdoor_left_steel_middle",{
	description = S("Steel Hoistway Door (left, middle - you hacker you!)"),
	tiles = {
		"celevator_transparent.png",
		"celevator_transparent.png",
		"celevator_door_glass_edge.png",
...
core.register_node("celevator:hwdoor_left_steel_top",{
	description = S("Steel Hoistway Door (left, top - you hacker you!)"),
	tiles = {
		"celevator_door_glass_edge.png",
		"celevator_transparent.png",
		"celevator_door_glass_edge.png",
...
core.register_node("celevator:hwdoor_right_steel_bottom",{
	description = S("Steel Hoistway Door (right, bottom - you hacker you!)"),
	tiles = {
		"celevator_transparent.png",
		"celevator_door_glass_edge.png",
		"celevator_door_glass_edge.png",
...
core.register_node("celevator:hwdoor_right_steel_middle",{
	description = S("Steel Hoistway Door (right, middle - you hacker you!)"),
	tiles = {
		"celevator_transparent.png",
		"celevator_transparent.png",
		"celevator_door_glass_edge.png",
...
core.register_node("celevator:hwdoor_right_steel_top",{
	description = S("Steel Hoistway Door (right, top - you hacker you!)"),
	tiles = {
		"celevator_door_glass_edge.png",
		"celevator_transparent.png",
		"celevator_door_glass_edge.png",
...
core.register_node("celevator:hwdoor_centersteel",{
	description = S("Center-Opening Steel Elevator Hoistway Door"),
	paramtype2 = "4dir",
	buildable_to = true,
	inventory_image = "celevator_door_centermetal_inventory.png",
	wield_image = "celevator_door_centermetal_inventory.png",
	...
	after_place_node = function(pos,player)
		...
		if carndef._root then
			celevator.get_meta(carpos):set_string("doortype","centermetal")
			celevator.doors.spawncardoors(carpos,core.fourdir_to_dir(newnode.param2),"centermetal",true)
		end
	end,
})

core.register_lbm({
	label = "Respawn hoistway door sills (center-opening steel)",
	name = "celevator:spawn_sill_centermetal",
	nodenames = {
		"celevator:hwdoor_left_steel_bottom",
		"celevator:hwdoor_right_steel_bottom",
	},
	run_at_every_load = true,
	action = celevator.doors.placesill,
})

celevator.doors.centermetal.spawncardoors = function(pos,dir,replace)
	local refs = {}
	for x=2,1,-1 do
		for y=1,3,1 do
			...
			local nname = string.format("celevator:hwdoor_%s_steel_%s",xnames[x],ynames[y])
			if replace then
...
function celevator.doors.centermetal.hwopen(pos,drivepos)
	...
	if not hwdoors_moving[hash] then
		...
		pmeta:set_string("doortype","centermetal")
...
function celevator.doors.centermetal.hwclose(pos,drivepos,nudge)
	celevator.doors.hwdoor_step_enabled = true
	local hwdoors_moving = core.deserialize(celevator.storage:get_string("hwdoors_moving")) or {}
	local hash = core.hash_node_position(pos)
...
function celevator.doors.centermetal.hwstep(hash,data,dtime,hwdoors_moving)
...
function celevator.doors.centermetal.caropen(pos)
...
function celevator.doors.centermetal.carclose(pos,nudge)
	celevator.doors.cardoor_step_enabled = true
	local cardoors_moving = core.deserialize(celevator.storage:get_string("cardoors_moving")) or {}
	local hash = core.hash_node_position(pos)
	...
	data.time = 0
	data.doortype = "centermetal"
	local erefs = celevator.doors.spawncardoors(pos,dir,"centermetal")
	local loffset = data.opendir
...
function celevator.doors.centermetal.carstep(hash,data,dtime,cardoors_moving)
	local present = celevator.doors.erefs[hash]
	if present then
		for i=1,6,1 do
	...
	if present then
		...
		elseif data.direction == "close" then
			...
			if data.time >= math.pi then
				...
				celevator.doors.spawncardoors(data.positions[1],fdir,"centermetal")
			end
		end
	else
		if data.direction == "open" then
		...
		elseif data.direction == "close" then
			...
			celevator.doors.spawncardoors(data.positions[1],vector.rotate_around_axis(fdir,vector.new(0,1,0),math.pi),"centermetal")
