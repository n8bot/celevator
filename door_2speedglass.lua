...
core.register_node("celevator:hwdoor_fast_glass_bottom",{
	...
	_celevator_doortype = "twospeedglass",
})
...
core.register_lbm({
	label = "Respawn hoistway door sills (2-speed glass)",
	name = "celevator:spawn_sill_twospeedglass",
	nodenames = {
...
celevator.doors.twospeedglass.spawncardoors = function(pos,dir,replace)
	local refs = {}
	for x=2,1,-1 do
		for y=1,3,1 do
...
function celevator.doors.twospeedglass.hwopen(pos,drivepos)
	...
	if not hwdoors_moving[hash] then
		...
		pmeta:set_string("doortype","twospeedglass")
...
function celevator.doors.twospeedglass.hwclose(pos,drivepos,nudge)
	celevator.doors.hwdoor_step_enabled = true
	local hwdoors_moving = core.deserialize(celevator.storage:get_string("hwdoors_moving")) or {}
	local hash = core.hash_node_position(pos)
...
function celevator.doors.twospeedglass.hwstep(hash,data,dtime,hwdoors_moving)
...
function celevator.doors.twospeedglass.caropen(pos)
...
function celevator.doors.twospeedglass.carclose(pos,nudge)
	celevator.doors.cardoor_step_enabled = true
	local cardoors_moving = core.deserialize(celevator.storage:get_string("cardoors_moving")) or {}
	local hash = core.hash_node_position(pos)
	...
	data.direction = "close"
	data.time = 0
	data.doortype = "twospeedglass"
	local erefs = celevator.doors.spawncardoors(pos,dir,"twospeedglass")
	local soffset = data.opendir
...
function celevator.doors.twospeedglass.carstep(hash,data,dtime,cardoors_moving)
	local present = celevator.doors.erefs[hash]
	if present then
		for i=1,6,1 do
	...
	if present then
		...
		elseif data.direction == "close" then
			...
			if data.time >= math.pi then
				...
				celevator.doors.spawncardoors(data.positions[1],fdir,"twospeedglass")
			end
		end
	else
		if data.direction == "open" then
		...
		elseif data.direction == "close" then
			...
			celevator.doors.spawncardoors(data.positions[1],vector.rotate_around_axis(fdir,vector.new(0,1,0),math.pi),"twospeedglass")
